# -*- coding: utf-8 -*-
"""
Revised on Sat Apr 1 09:07:23 2018
@author: Dirk
...
class MainApplication(tk.Frame):
    def __init__(self, parent):
        ...
        self.check = CheckButtonField(self)
        self.lab = LabelField(self, self.low, self.up, self.check)
...
class LabelPhotoField(tk.Frame):
    def __init__(self, parent):
        self.parent = parent
        self.img = tk.PhotoImage(file='gauss.gif')
        self.photoLabel = tk.Label(root, image=self.img)
...
class CheckButtonField(tk.Frame):
    def __init__(self, parent):
        self.parent = parent
        self.inverse = tk.IntVar()
        str = 'Take the inverse (white area under curve)'
        self.chkbut = tk.Checkbutton(root, text=str, offvalue=0,
                                     onvalue=1, variable=self.inverse)
        self.chkbut.grid(row=4, column=0, columnspan=2)
...
class LabelField(tk.Frame):
    def __init__(self, parent, fromEntryLow, fromEntryHigh, fromCheck):
        self.parent = parent
        self.fromEntryLow = fromEntryLow
        self.fromEntryHigh = fromEntryHigh
        self.check = fromCheck
    ...
    def extract(self):
        ...
        if self.check.inverse.get():
            self.label.config(text='Gaussian probability: ' +
                              str(round(1-result, 4)))
        else:
            self.label.config(text='Gaussian probability: ' +
                              str(round(result, 4)))
